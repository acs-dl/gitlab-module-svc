...
func (g *gitlab) GetSubgroupsFomApi(link string) ([]data.Sub, error) {
	response, err := helpers.MakeRequestWithPagination(data.RequestParams{
		Method: http.MethodGet,
		Link:   fmt.Sprintf("https://gitlab.com/api/v4/groups/%s/subgroups", regexp.MustCompile("/").ReplaceAllString(link, "%2F")),
		Body:   nil,
